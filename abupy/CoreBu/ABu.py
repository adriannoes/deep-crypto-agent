...
def run_loop_back(read_cash, buy_factors, sell_factors, stock_picks=None, choice_symbols=None, n_folds=2,
                  ...
                  n_process_pick=None):
    """
    Execute timing and stock selection backtesting.

    It is recommended to use abu.run_kl_update() function to update data first before using
    abu.run_loop_back() function for full market backtesting. In run_kl_update(), it will
    first force the use of network data for updates, and after the update is complete, change
    the data acquisition method to local cache. The benefit of using abu.run_kl_update() is
    to separate data updates from strategy backtesting, which will bring positive improvements
    in running efficiency and problem troubleshooting.

    :param read_cash: Initial capital amount, e.g.: 1000000
    :param buy_factors: Buy factor strategy sequence for backtesting,
                    e.g.:
                        buy_factors = [{'xd': 60, 'class': AbuFactorBuyBreak},
                                       {'xd': 42, 'class': AbuFactorBuyBreak}]
    :param sell_factors: Sell factor sequence for backtesting,
                    e.g.:
    ...
    :param stock_picks: Stock selection factor sequence for backtesting:
                    e.g.:
                        stock_pickers = [{'class': AbuPickRegressAngMinMax,
                                         ...
                                         {'class': AbuPickStockPriceMinMax,
                                          ...
                                          'reversed': False}]
    :param choice_symbols: Candidate stock pool, default is None, which uses the market type
                            of abupy.env.g_market_target for full market backtesting.
                            If not None, it should be a symbol sequence.
                    e.g.:
                        choice_symbols = ['usNOAH', 'usSFUN', 'usBIDU', 'usAAPL', 'usGOOG',
                                          'usTSLA', 'usWUBA', 'usVIPS']
    :param n_folds: int, backtest n_folds years of historical data
    :param start: Backtest start time, str object, e.g.: '2013-07-10'
    :param end: Backtest end time, str object, e.g.: '2016-07-26'
    :param commission_dict: Passed to AbuCapital for custom trading commission.
                    e.g.:
    ...
    :param n_process_kl: Number of parallel processes for financial time series data collection,
                         default None, internally allocated based on CPU count
    :param n_process_pick: Number of parallel processes for timing and stock selection operations,
                           default None, internally allocated based on CPU count
    :return: (AbuResultTuple object, AbuKLManager object)
    """
