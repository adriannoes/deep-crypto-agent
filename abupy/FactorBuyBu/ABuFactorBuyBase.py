# -*- encoding:utf-8 -*-
"""
    Buy timing strategy factor base module
"""
...
class BuyCallMixin(object):
    """
    Mixin class representing buy call (bullish). Not exactly the concept of buy call in options,
    but represents bullish forward operations, i.e., expecting the trading target price to rise
    after buying, with rising prices bringing profits.
    """

    @LazyFunc
    def buy_type_str(self):
        """Used to distinguish buy type, unique value is 'call'"""
        return "call"

    @LazyFunc
    def expect_direction(self):
        """Expected profit direction, 1.0 means forward expectation"""
        return 1.0


class BuyPutMixin(object):
    """
    Mixin class representing buy put (bearish). Application scenarios are in options and futures strategies.
    Not exactly the concept of buy put in options, but represents bearish reverse operations,
    i.e., expecting the trading target price to fall after buying, with falling prices bringing profits.
    """

    @LazyFunc
    def buy_type_str(self):
        """Used to distinguish buy type, unique value is 'put'"""
        return "put"

    @LazyFunc
    def expect_direction(self):
        """Expected profit direction, -1.0 means reverse expectation"""
        return -1.0


class AbuFactorBuyBase(six.with_metaclass(ABCMeta, AbuParamBase)):
    """
    Buy timing strategy factor base class: Each subclass inheriting from AbuFactorBuyBase must
    mix in one direction class, and only one direction class. That is, a specific buy factor must
    have a clear buy direction, and can only have one buy direction. A factor cannot be both bullish
    and bearish at the same time.

    Buy factors can internally accommodate dedicated sell factors and stock selection factors,
    i.e., sell factors and stock selection strategies that only take effect for the source factor,
    and stock selection factors dynamically re-select stocks monthly or weekly within the timing
    cycle according to the strategy.
    """

    def __init__(self, capital, kl_pd, combine_kl_pd, benchmark, **kwargs):
        """
        :param capital: AbuCapital instantiated object for capital management
        :param kl_pd: Financial time series for timing period, pd.DataFrame object
        :param combine_kl_pd: Financial time series merged with previous year's time series, pd.DataFrame object
        :param benchmark: Trading benchmark object, AbuBenchmark instance object. Factors can optionally
                          use it, such as market comparison and other functions.
        """
        # 择时金融时间序列走势数据
        self.kl_pd = kl_pd
